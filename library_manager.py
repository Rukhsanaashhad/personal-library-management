...
def get_library_stats():
    ...
    read_books = sum(1 for book in st.session_state.library['read status'])
...
if lottie_book:
    with st.sidebar:
            st_lottie(lottie_book, height=200, key="book assimilation")
...
st.markdown("<h1 class='main-header'>Personal Library Manager By Muhammad Ashhad</h1>", unsafe_allow_html=True)
if st.session_state.current_view == "add":
    ...
    with st.form(key="add_book_form"):
        ...
        with col1:
            title = st.text_input("Book Title", max_chars=1000)
            author = st.text_input("Author", max_chars=1000)
...
""",unsafe_allow_html=True)
            ...
            if stats['authors']:
                st.markdown("<h1>Top Authors </h1>", unsafe_allow_html=True)
...
st.markdown("-----")
st.markdown("Copyright 2025 Muhammad Ashhad Khan Personal Library Manager", unsafe_allow_html=True)
